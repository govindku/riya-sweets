...
const getDeliveryCharge = () => {
  ...
  const charge = Number(savedSettings.deliveryCharge);

  return Number.isFinite(charge) && charge >= 0 ? charge : 50;
};
...
function Order() {
  ...
  const [placedDeliveryCharge, setPlacedDeliveryCharge] = useState(0);
  ...
  const updateQuantity = (id, change) => {
    ...
    localStorage.setItem("riyaOrder", JSON.stringify(updatedItems));
  };
  ...
  const calculateDelivery = (
    ...
    if (!address || !city || !/^\d{6}$/.test(pincode)) {
      return;
    }

    setDeliveryCharge(getDeliveryCharge());
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      calculateDelivery(address, city, pincode);
    }, 400);
  ...
  const saveUserDetails = (loggedInUser) => {
  ...
  const saveOrder = async (
    ...
    const newOrder = {
      ...
      customer: {
        name: customer.name,
        phone: customer.phone,
        email: loggedInUser.email || customer.email,
        address: customer.address,
        city: customer.city,
        pincode: customer.pincode,
      },
    ...
    window.dispatchEvent(new Event("riyaOrderUpdated"));
    ...
    const supabaseOrder = {
      ...
      user_id: loggedInUser.email || customer.email || null,
      ...
      customer_email:
        loggedInUser.email || customer.email || null,
  ...
  const startRazorpayPayment = async (loggedInUser) => {
    try {
      ...
      if (!response.ok || !razorpayOrder.success) {
        throw new Error(
      ...
      const options = {
        ...
        description: "Food Order + Delivery Payment",
        ...
        prefill: {
          ...
          email:
            loggedInUser.email || customer.email,

          contact: customer.phone,
        },

        notes: {
          restaurant: "Riya Sweets",
        ...
        handler: async function (paymentResponse) {
          try {
            ...
            localStorage.removeItem("riyaOrder");
      ...
      const message =
        error?.message === "Failed to fetch"
          ? "Payment server se connection nahi ho raha. Please make sure server is running on port 5000."
          : error?.message ||
  ...
  const handleSubmit = async (e) => {
    ...
    if (deliveryCharge <= 0) {
      alert("Unable to calculate a valid delivery charge.");
      return;
    }

    await startRazorpayPayment(loggedInUser);
  };
      ...
      <section className="order-hero">
        ...
        <div className="order-hero-content">
          ...
          <h1 className="order-hero-title">
            Complete Your <i>Order</i>
          </h1>

          <p>
            Almost there. Just a few details and
            your delicious meal will be on its way.
          </p>
        </div>
      </section>
        ...
        <aside className="order-summary">
          ...
          {orderItems.length === 0 ? (
            <div className="order-empty">
              ...
              <p>
                Go back to the menu and add something
                delicious.
              </p>
            ...
            <>
              ...
              <div className="order-secure">
                ...
                <p>
                  ...
                  Your final payment includes food and
                  delivery charges.
                </p>
              </div>
            </>
